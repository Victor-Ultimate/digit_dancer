    ...
    <el-container>
      <el-main>
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="login_ruleForm">
          ...
          <el-form-item label="验证" prop="slidevalid">
            <slide-verify :l="42"
                          :r="10"
                          :w="310"
                          :h="155"
  ...
  data() {
    const validateusername = (rule, value, callback) => {
      ...
      if (value === '') {
        ...
        callback(new Error('该账号未注册1！'));
      } else {
        var having = false
        this.$axios.post('http://localhost:3000/user/account/utils/checkuser/', this.ruleForm.username).then(res => {
              console.log('res=>', res);
              having = res.data.data
              if (!having) {
                // callback(new Error('该账号未注册2！'));
                callback(new Error(having));
              }else{
                callback();
              }
            }
    ...
    const validatepassword = (rule, value, callback) => {
      ...
      if (value === '') {
        ...
        callback(new Error('密码或昵称错误，请重新输入1！'));
      } else {
        var match = false
        this.$axios.post('http://localhost:3000/user/account/utils/checklogin/', {username:this.ruleForm.username, password:this.ruleForm.password}).then(res => {
              console.log('res=>', res);
              match = res.data.data
              if (!match) {
                callback(new Error('密码或昵称错误，请重新输入2！'));
              }else{
                callback();
              }
            }
