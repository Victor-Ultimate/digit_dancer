  ...
  data() {
    var validateusername = (rule, value, callback) => {
      ...
      if (value === '') {
        ...
        var having = false;
        this.$axios.post('http://localhost:3000/user/account/utils/checkrcheckuser/', {username:this.ruleForm.username}).then(res => {
              console.log('res=>', res);
    ...
    var validateemail = (rule, value, callback) => {
      ...
      if (value === '') {
        ...
        var having = false;
        this.$axios.post('http://localhost:3000/user/account/utils/checkemail/', {email:this.ruleForm.email}).then(res => {
              console.log('res=>', res);
              having = res.data.data
            }
        )
  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
            if (valid) {
              //发起post请求
              this.$axios.post('http://localhost:3000/user/account/register/', this.ruleForm).then(res => {
                    if (res.data.message === '') {
